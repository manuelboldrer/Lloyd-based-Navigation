function [wp_smart, index_min, wp_path, Tend, flag] = generate_wp_path(waypoints, x, y, index_prev, kk, obs)
flag         = 0;
L_calculus   = 3; % the largest curvilinear distance from the wp, i.e., max preview lenght
L_min        = 1; % minimum preview lenght
max_distance = 1; % if the closest wp is further than this, return it as wp_smart
% angle_max    = pi/3; % max tolerated angle to compute varying preview


Tend         = kk;
distance_wp_path     = .1;  % a wp per distance_wp_path meters
...
%     index_wp_smart = min(floor(index_min + L_min/distance_wp_path), size(wp_path,2));
% end
% if wp_path(:, index_wp_smart) == wp_path(:, end)
%     wp_smart = wp_path(:, end);
% else
    ...
    % end
% end
while( noCollision([x,y]',[wp_smart(1),wp_smart(2)]',obs)==0 )
    index_wp_smart = index_wp_smart-1;
    wp_smart       = wp_path(:, index_wp_smart);
    %     if index_wp_smart == 0
    %         index_wp_smart = 1;
    %             wp_smart       = wp_path(:, index_wp_smart);
