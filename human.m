 function output = human(x,y,vx,vy,sizeO)
...
darkblue = [0 0 0.5];
p1= patch('Faces',[1:length(alpha)],'Vertices',[feet2(1,:)',feet2(2,:)'], 'FaceColor','y','EdgeColor','y');
p2= patch('Faces',[1:length(alpha)],'Vertices',[feet1(1,:)',feet1(2,:)'], 'FaceColor','y','EdgeColor','y');
p3= patch('Faces',[1:length(alpha)],'Vertices',[body(1,:)',body(2,:)'], 'FaceColor',[0 .5 .5],'EdgeColor','k');
p4= patch('Faces',[1:length(alpha)],'Vertices',[head(1,:)',head(2,:)'], 'FaceColor','y','EdgeColor','y');
p5= patch('Faces',[1:length(alpha)],'Vertices',[noise(1,:)',noise(2,:)'], 'FaceColor','y','EdgeColor','y');
p6 = circle(x,y,sizeO,DodgerBlue);
output = [p1,p2,p3,p4,p5,p6,h,h1,h2,h3,h4,h5,h6]; 
% output = [p6];